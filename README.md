# cuda-energy

ATP/energy system for agent fleet — budgets, apoptosis, circadian rhythm, epigenetic memory (Rust)

Part of the Cocapn biology layer — bio-inspired agent lifecycle and energy.

## What It Does

### Key Types

- `EnergyBudget` — core data structure
- `EnergyCosts` — core data structure
- `CircadianRhythm` — core data structure
- `EpigeneticMemory` — core data structure
- `ApoptosisProtocol` — core data structure
- `FleetEnergy` — core data structure

## Quick Start

```bash
# Clone
git clone https://github.com/Lucineer/cuda-energy.git
cd cuda-energy

# Build
cargo build

# Run tests
cargo test
```

## Usage

```rust
use cuda_energy::*;

// See src/lib.rs for full API
// 12 unit tests included
```

### Available Implementations

- `EnergyBudget` — see source for methods
- `Default for EnergyCosts` — see source for methods
- `CircadianRhythm` — see source for methods
- `EpigeneticMemory` — see source for methods
- `ApoptosisProtocol` — see source for methods
- `FleetEnergy` — see source for methods

## Testing

```bash
cargo test
```

12 unit tests covering core functionality.

## Architecture

This crate is part of the **Cocapn Fleet** — a git-native multi-agent ecosystem.

- **Category**: biology
- **Language**: Rust
- **Dependencies**: See `Cargo.toml`
- **Status**: Active development

## Related Crates

- [cuda-neurotransmitter](https://github.com/Lucineer/cuda-neurotransmitter)
- [cuda-biology](https://github.com/Lucineer/cuda-biology)
- [cuda-genepool](https://github.com/Lucineer/cuda-genepool)
- [cuda-dna](https://github.com/Lucineer/cuda-dna)

## Fleet Position

```
Casey (Captain)
├── JetsonClaw1 (Lucineer realm — hardware, low-level systems, fleet infrastructure)
├── Oracle1 (SuperInstance — lighthouse, architecture, consensus)
└── Babel (SuperInstance — multilingual scout)
```

## Contributing

This is a fleet vessel component. Fork it, improve it, push a bottle to `message-in-a-bottle/for-jetsonclaw1/`.

## License

MIT

---

*Built by JetsonClaw1 — part of the Cocapn fleet*
*See [cocapn-fleet-readme](https://github.com/Lucineer/cocapn-fleet-readme) for the full fleet roadmap*
